...
@csrf_exempt
def requestLock():
    centralizedLockPeer = "0.0.0.0:5000"
...
def castNewVote(request, candidateId):
    if candidateId is not None:
        ...
        voted = currentVoter.has_voted
        
        if voterId in dic:
            return HttpResponse("You have already voted once!!!")
        if currentVoter is not None:
            ...
            if checkLock is True:
                ...
                for thread in threads:
                    thread.start()
                response = threads[0].join()
                for i in range(1,len(threads)):
                    response = threads[i].join()
